#!/usr/bin/env python
"""Analyze the paired seed-123 Phase-31 causal-EMA screen."""
...
RESULT_ROOT = ROOT / "results" / "phase31_dynamic_ema_screen"
SEED = 123
STEPS = 15_000
ARMS = (
    "rope-fixed",
...
CONTRASTS = (
    ("clock-pointwise", "rope-fixed", "pointwise clock value"),
    ("clock-ema", "rope-fixed", "EMA clock total value"),
    ("clock-ema", "clock-pointwise", "EMA clock incremental value"),
    ("addrope-position", "rope-fixed", "position-only AddRoPE value"),
    (
        "addrope-content-pointwise",
        "addrope-position",
        "pointwise content incremental value",
    ),
    (
        "addrope-content-ema",
        "addrope-content-pointwise",
        "EMA memory incremental value",
    ),
    (
        "addrope-content-ema",
        "addrope-position",
        "EMA content total incremental value",
    ),
)


def run_dir(arm: str) -> Path:
    return OUTPUT_ROOT / f"phase31-{arm}-seed{SEED}-s{STEPS}-h768d8"
...
def analyze() -> dict:
    ...
    if missing:
        raise RuntimeError(
            "Phase-31 screen is incomplete; missing markers:\n" + "\n".join(missing)
        )
    ...
    return {
        "scope": "single_seed_15k_mechanism_screen",
        "seed": SEED,
        ...
        "clock_speed_bound": 0.01,
        "ema_decay_init": 0.9,
        "ema_definition": "learned per-latent bias-corrected causal EMA",
        "triage_thresholds": {
            "survive": "delta <= -0.01",
            "prune": "delta >= +0.01",
            "unresolved": "otherwise",
            "note": "single-seed screen; replicate only clear survivors",
        },
        "arms": arms,
        "contrasts": contrasts,
    }
...
def main() -> None:
    ...
    (RESULT_ROOT / "phase31_analysis.json").write_text(
        json.dumps(results, indent=2, sort_keys=True) + "\n"
    )
    (RESULT_ROOT / "PHASE31_RESULTS.md").write_text(rendered)
    print(rendered)
