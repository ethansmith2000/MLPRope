...
ablation shows that rotating an already indexed additive carrier changes both
its phase composition and its order relative to learned query-key gains. We
use these observations to study how
...
composition in Equation~\ref{eq:postrope}, while also potentially reflecting
the changed order of learned query-key gains and rotation.
...
\begin{enumerate}
    ...
    \item counterfactual changes after preserving only or removing only the
    positional mean of a learned direct carrier, and after zeroing its direct
    and scalar branches, explicitly labeled as inference-time distribution
    shifts.
\end{enumerate}
...
final version will expand this
