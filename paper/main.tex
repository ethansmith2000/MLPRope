...
and bf16 arithmetic. The preliminary cohort uses eight sequences per update
for 200k optimizer steps (1.638B nominal tokens). The matched paper cohort
uses 32 sequences per update for 100k steps (3.277B nominal tokens), selected
by a predeclared throughput and memory benchmark. Results from the two batch
regimes are reported separately rather than pooled as matched runs.
...
composition in Equation~\ref{eq:postrope}.
...
The mature evidence currently uses one corpus, one fixed training context, two model
...
suggest that application site and composition order are important. The
remaining experiments isolate the contribution of RoPE, learned layerwise
gating, and repeated per-block access, then test whether the effect survives
data shifts, a larger-batch paper recipe, modern architectural choices, scale,
and two-dimensional spatial attention.
