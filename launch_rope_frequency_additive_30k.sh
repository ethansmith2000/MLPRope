#!/bin/bash
# Queue phase-22 through at most two sequential gpu-claim workers.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_DIR="${SCRIPT_DIR}/sweep_configs/phase22_rope_additive_30k"
OUTPUT_ROOT="${SCRIPT_DIR}/model-output/position_bias_phase22_rope_additive_30k"
LOG_DIR="${SCRIPT_DIR}/logs/phase22_rope_additive_30k"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/prepare_rope_frequency_additive_30k.py"

CONFIGS=()
for seed in 123 456 789; do
  for arm in fixed additive; do
    CONFIGS+=("${CONFIG_DIR}/phase22-${arm}-seed${seed}-s30000-h768d8.json")
  done
done
...
echo "MLPROPE_PHASE22_STARTED $(date -u +%Y-%m-%dT%H:%M:%SZ) workers=${MAX_WORKERS} gpus=${GPU_SELECTOR}"
...
if [[ "${failed}" -ne 0 ]]; then
  echo "One or more phase-22 jobs failed" >&2
  exit 1
fi

"${PYTHON_BIN}" "${SCRIPT_DIR}/analyze_rope_frequency_additive_30k.py"
echo "MLPROPE_PHASE22_COMPLETED $(date -u +%Y-%m-%dT%H:%M:%SZ)"
