#!/bin/bash
# Queue phase-23 through at most two sequential gpu-claim workers.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_DIR="${SCRIPT_DIR}/sweep_configs/phase23_dynamic_frequency"
OUTPUT_ROOT="${SCRIPT_DIR}/model-output/position_bias_phase23_dynamic_frequency"
LOG_DIR="${SCRIPT_DIR}/logs/phase23_dynamic_frequency"
...
if [[ "${MAX_WORKERS}" != "1" && "${MAX_WORKERS}" != "2" ]]; then
  echo "MAX_WORKERS must be 1 or 2 for this screen" >&2
  exit 2
fi
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/prepare_rope_dynamic_frequency_screen.py"

CONFIGS=()
for seed in 123 456 789; do
  for arm in linear-horizon lowrank-linear-horizon lowrank-silu-horizon; do
    CONFIGS+=("${CONFIG_DIR}/phase23-${arm}-seed${seed}-s5000-h768d8.json")
  done
done
...
echo "MLPROPE_PHASE23_STARTED $(date -u +%Y-%m-%dT%H:%M:%SZ) workers=${MAX_WORKERS} gpus=${GPU_SELECTOR}"
...
if [[ "${failed}" -ne 0 ]]; then
  echo "One or more phase-23 jobs failed" >&2
  exit 1
fi

"${PYTHON_BIN}" "${SCRIPT_DIR}/analyze_rope_dynamic_frequency_screen.py"
echo "MLPROPE_PHASE23_COMPLETED $(date -u +%Y-%m-%dT%H:%M:%SZ)"
