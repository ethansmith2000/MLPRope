#!/usr/bin/env python
"""Generate the frozen Phase-59 modern-backbone transfer configs."""
...
PREFLIGHT_OUTPUT_ROOT = (
    ROOT / "model-output" / "position_bias_phase59_modern_backbone_preflight"
)
FINAL_START = 9_216
...
def _arm_payload(arm: str) -> dict:
    ...
    if arm == "scalar-qkpre":
        ...
        raise ValueError(f"Unknown Phase-59 arm: {arm}")
    return payload
    ...
    payload.update(
        {
            ...
            "wandb_group": "phase59-modern-backbone-b32-s100000",
        }
    )
    return payload
...
def generate() -> list[Path]:
    ...
    for index, arm in enumerate(ARMS):
        run_name = f"phase59-modern-{arm}-lr1p2e3-seed123-b32-s100000-h768d8"
        ...
        preflight_name = (
            f"phase59-preflight-modern-{arm}-lr1p2e3-seed123-b32-s100-h768d8"
        )
        ...
        preflight.update(
            {
                ...
                "wandb_group": "phase59-modern-backbone-preflight",
            }
        )
