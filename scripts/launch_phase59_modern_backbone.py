#!/usr/bin/env python
"""Run Phase-59 preflights or main jobs through gpu-claim, cap two."""
...
LOG_ROOT = ROOT / "logs" / "phase59_modern_backbone"
RESULT_ROOT = ROOT / "results" / "phase59_modern_backbone"
OUTPUT_ROOT = ROOT / "model-output" / "position_bias_phase59_modern_backbone"
...
def _finite_run_logs(config_path: Path) -> bool:
    config = _config(config_path)
    output_dir = Path(config["output_dir"])
    if not _finite_jsonl(output_dir / "metrics.jsonl"):
        return False
    qk_preprojection = config.get("qk_preprojection", {})
    if qk_preprojection.get("enabled") and qk_preprojection.get("learnable_gate"):
        return _finite_jsonl(output_dir / "intervention_optimization.jsonl")
    return True


def _endpoint_path(config: dict) -> Path:
    return (
...
def _cleanup_recovery(config_path: Path) -> dict:
    """Remove only verified recovery checkpoints from one completed main run."""
    config = _config(config_path)
    run_dir = Path(config["output_dir"]).resolve()
    if run_dir.parent != OUTPUT_ROOT.resolve() or run_dir.is_symlink():
        raise RuntimeError(f"Unsafe Phase-59 run directory: {run_dir}")
    if not _complete(config_path) or not _endpoint_path(config).is_file():
        raise RuntimeError(f"Refusing cleanup before verified completion: {run_dir}")

    checkpoints = sorted(run_dir.glob("step_*"))
    if not checkpoints:
        print(f"cleanup {config['run_name']}: no recovery checkpoints remain", flush=True)
        return {
    ...
    try:
        while pending or processes:
            ...
            for process, (path, run_name, handle) in list(processes.items()):
                ...
                elif not _finite_run_logs(path):
                    failures.append((run_name, 1, "missing or non-finite run logs"))
                elif not args.preflight and not _endpoint_path(_config(path)).is_file():
