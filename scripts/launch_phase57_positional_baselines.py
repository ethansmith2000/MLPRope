#!/usr/bin/env python
"""Run Phase-57 preflights or main jobs with a hard two-GPU cap."""
...
LOG_ROOT = ROOT / "logs" / "phase57_positional_baselines"
...
def _cleanup_recovery(config_path: Path) -> dict:
    """Remove only recovery checkpoints from one exactly completed main run."""
    config = json.loads(config_path.read_text())
    run_dir = Path(config["output_dir"]).resolve()
    expected_root = (ROOT / "model-output" / "position_bias_phase57_positional_baselines").resolve()
    if run_dir.parent != expected_root or run_dir.is_symlink():
        raise RuntimeError(f"Unsafe Phase-57 run directory: {run_dir}")
    marker = json.loads((run_dir / "COMPLETED").read_text())
    if int(marker.get("completed_steps", -1)) != int(config["max_train_steps"]):
        raise RuntimeError(f"Refusing cleanup for incomplete run: {run_dir}")
    detail = run_dir / "evaluation_details" / "step_00100000_context_001024.json"
    if not detail.is_file():
        raise RuntimeError(f"Refusing cleanup without final evaluation details: {detail}")
    ...
    event = {
        ...
        "run_name": config["run_name"],
        "reason": "run completed and final evaluation details exist; checkpoints were recovery-only",
        "recoverable": False,
        "removed": removed,
        ...
        while pending or processes:
            ...
            for process, (path, run_name, handle) in list(processes.items()):
                ...
                if not args.preflight:
                    try:
                        _cleanup_recovery(path)
                    except Exception as error:  # preserve state and fail closed
