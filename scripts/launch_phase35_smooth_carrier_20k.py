#!/usr/bin/env python
"""Queue Phase-35 preflights or long runs through the shared GPU claimer."""
...
CONFIG_ROOT = REPO_DIR / "sweep_configs" / "phase35_smooth_carrier_20k"


def _config_target(path: Path) -> tuple[str, Path, int]:
    # Resolve inheritance through the authoritative loader, rather than
    # duplicating its deep-merge behavior here.
...
def main() -> int:
    ...
    if len(configs) != 8:
        raise SystemExit(f"Expected eight Phase-35 configs, found {len(configs)}")
    ...
    phase = "preflight" if args.preflight else "20k"
    log_dir = REPO_DIR / "logs" / "phase35_smooth_carrier_20k" / phase
