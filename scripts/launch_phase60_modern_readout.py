#!/usr/bin/env python
"""Run Phase-60 preflights or main jobs through gpu-claim, cap two."""
...
LOG_ROOT = ROOT / "logs" / "phase60_modern_readout"
...
def _cleanup_recovery(config_path: Path) -> dict:
    ...
    if run_dir.parent != OUTPUT_ROOT.resolve() or run_dir.is_symlink():
        raise RuntimeError(f"Unsafe Phase-60 run directory: {run_dir}")
    ...
    if not checkpoints:
        ...
        return {"run_name": config["run_name"], "removed": [], "reclaimed_bytes": 0}
    removed = []
    reclaimed = 0
