#!/usr/bin/env python
"""Delete only completed Phase-55 rolling recovery states after analysis."""
...
def main() -> None:
    if not (RESULT_ROOT / "summary.json").is_file():
        raise RuntimeError("Refusing cleanup before Phase-55 analysis completes")
    removed = []
    reclaimed = 0
    for config_path in sorted(CONFIG_ROOT.glob("*.json")):
        ...
        if int(marker.get("completed_steps", -1)) != int(config["max_train_steps"]):
            raise RuntimeError(f"Incomplete Phase-55 run: {run_dir}")
    ...
    payload = {
        "scope": "phase55_completed_recovery_cleanup",
        "removed": removed,
        "reclaimed_bytes": reclaimed,
        "recoverable": False,
        "reason": "All Phase-55 final evaluations and aggregate analysis completed; rolling states had recovery-only purpose.",
    }
