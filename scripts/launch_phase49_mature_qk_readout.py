#!/usr/bin/env python
"""Run Phase 49 through the shared lifetime GPU claimer."""
...
LOG_DIR = REPO_DIR / "logs" / "phase49_mature_qk_readout"
EXPECTED_ARMS = 5
...
def _run(*, preflight: bool, owner: str, gpu: str) -> int:
    ...
    if len(configs) != EXPECTED_ARMS:
        raise SystemExit(
            f"Expected {EXPECTED_ARMS} configs in {config_dir}, found {len(configs)}"
        )
