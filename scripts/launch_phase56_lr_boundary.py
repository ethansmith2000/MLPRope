#!/usr/bin/env python
"""Run Phase-56 preflights or main jobs with a hard two-GPU cap."""
...
LOG_ROOT = ROOT / "logs" / "phase56_lr_boundary"
...
def main() -> int:
    ...
    if len(configs) != 2:
        raise SystemExit(f"Expected two configs in {config_dir}, found {len(configs)}")
    ...
    if failures:
        for run_name, code in failures:
            print(f"FAILED {run_name} rc={code}", file=sys.stderr)
        return 1
    return 0
